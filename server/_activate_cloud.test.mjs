...
ok(db.prepare('SELECT COUNT(*) n FROM cloud_push_state WHERE done = 1').get().n === 15, '15 tables marquées terminées (reprise)', db.prepare('SELECT COUNT(*) n FROM cloud_push_state WHERE done = 1').get().n);
