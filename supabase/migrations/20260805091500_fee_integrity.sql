...
-- « Tient la caisse » = rôle admin/censeur, OU compte délégué à qui la page
-- Frais a été explicitement accordée. Les permissions granulaires
-- (school_users.permissions, tableau JSON de routes) sont ADDITIVES au rôle
-- dans l'app : une policy qui ne regarderait que le rôle priverait d'encaisser
-- un compte que l'établissement a pourtant habilité.
--
-- La comparaison se fait sur le TEXTE, sans cast jsonb : un `permissions` mal
-- formé ferait lever la fonction, donc échouer TOUTE écriture de recette. Une
-- correspondance un peu large vaut mieux qu'une caisse bloquée.
