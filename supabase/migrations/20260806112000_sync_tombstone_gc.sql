...
-- État initial, en NOTICE (db push n'affiche pas les SELECT).
DO $$
DECLARE r record; n int := 0;
BEGIN
  FOR r IN SELECT ecole, tombstones, curseur_lan, etat
             FROM public.v_sync_tombstone_health
            WHERE tombstones > 0 ORDER BY tombstones DESC LIMIT 8 LOOP
    RAISE NOTICE 'ÉCOLE % | % tombstones | curseur=% | %', r.ecole, r.tombstones, COALESCE(r.curseur_lan::text,'—'), r.etat;
    n := n + 1;
  END LOOP;
  IF n = 0 THEN RAISE NOTICE 'Aucune école avec tombstones.'; END IF;
  SELECT count(*) INTO n FROM public.sync_tombstones;
  RAISE NOTICE 'TOTAL tombstones = % | purge INERTE (aucun curseur enregistré tant que sync-pull patché n''est pas déployé)', n;
END $$;
